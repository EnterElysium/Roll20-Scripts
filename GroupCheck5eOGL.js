  ...
  const version = "1.12",
    ...
    outputStyle = (() => {
      const makeBox = (header, subheader, freetext, content) => {
          return "<div class='sheet-rolltemplate-npcaction groupcheck'><div class='sheet-container groupcheck'>" +
            `<div class="sheet-row sheet-header"><span>${header}</span></div>` +
            `<div class="sheet-row sheet-subheader"><span class="sheet-italics">${subheader || ""}</span></div>` +
            `<div class="sheet-arrow-right"></div>` +
            `<table class="sheet-desc" style="width:100%">${content}</table>` +
            (freetext ? `<div style="text-align:center;">${freetext}</div>` : "") +
            "</div></div>";
        },
        makeRow = (pic, name, roll1, roll2, isLast) => {
          return `<tr${isLast ? "" :" style=\"border-bottom: 0.3px solid #7e2d40\""}>` +
            makeName(pic, name) +
            `<td style="text-align:center;width:36px;">${roll1}</td>` +
            (roll2 ? `<td style="text-align:center;width:33px;">${roll2}</td>` : "") +
            "</tr>";
        },
        makeName = (pic, name) => {
          const imgStyle = "display:inline-block;height:30px;width:30px;vertical-align:middle";
          return "<td style=\"padding:3px;height:30px;width:36px;\">" +
            (pic ? `<div style="${imgStyle};background:url('${pic}') 0/contain no-repeat"></div>` : "") +
            `</td><td style="width:100%;padding:0px 3px;"><span style="vertical-align:middle;font-weight:normal;font-size:12px;text-align:left;">${name}</span>` +
            "</td>";
        },
        makeCommandButton = (name, command, useBorder) => {
          const style = `style='font-family:"Times New Roman",Times,serif;font-variant:small-caps;color:#7e2d40;font-size:14px;background-color:transparent;margin:-10px 0px;'`;
