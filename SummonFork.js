    ...
    const handleInput = function(msg) {
        try {
            if(msg.type=="api" && msg.content.indexOf("!Spawn") === 0 ) {
                ...
                //log("Obj: "+msg.selected+" | Obj: "+typeof msg.selected+" | Obj: "+msg.selected._id);
				if (msg.selected === undefined && msg.content.indexOf("memento") === -1 && playerIsGM(msg.playerid)==false) {
                    sendChat('SpawnAPI',`/w "${whoDat}" `+ 'You must select a token to proceed');
                    return;
                }
				
				//PICKUPHERE Turns out msg.selected is not a full array but just an array of objects which each contain only the properties _id and _type
				var spawntokens = findObjs({
					_name:"SpawnPoint",
					_type:"graphic",
					_pageid:Campaign().get("playerpageid")
				},{caseInsensitive: true});
				spawntokens[0]===undefined ? log("ST undef") : log("ST def");
				log(JSON.stringify(spawntokens[0]));
				var spawnToken = _.pick(spawntokens[0],'_id','_type');
				// msg.selected = spawnToken;
				log("Obj: "+msg.selected+" | Obj: "+typeof msg.selected+" | Obj str: "+JSON.stringify(msg.selected));
				log("Obj: "+spawnToken+" | Obj: "+typeof spawnToken+" | Obj str: "+JSON.stringify(spawnToken));
				
                if (msg.selected === undefined && msg.content.indexOf("memento") === -1 && playerIsGM(msg.playerid)!==false) {
                    sendChat('SpawnAPI',`/w "${whoDat}" `+ 'GM, you must select a token to proceed');
                    return;
                }
